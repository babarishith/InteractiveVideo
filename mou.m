...
cen = [pos(3)/2 pos(4)/2];
handles.ang = atan2(cen(2) - y, cen(1) - x)*180/pi + 180;
% im = main(handles.img, ang);
set(handles.angle, 'string', ['angle:' num2str(handles.ang)]); % update text for x loc
% figure(handles.back)
% imshow(handles.img);
handles.output = hObject;
guidata(hObject, handles);
...
set(handles.angle, 'string', ['angle:' num2str(handles.ang)]); % update text for x loc
disp "end"
set(handles.status, 'string', ['Rotated by:' num2str(handles.ang)]);
handles.output = hObject;
guidata(hObject, handles);
